...
import CredibleNotification from './CredibleNotification';
...
export default function Results({ isVisible, onCTAClick }) {
  ...
  const trackCTAClick = (ctaPosition) => {
    if (typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'cta_click', {
        ...
        user_pattern: scoreCategory?.pattern
      });
    }
    console.log('CTA clicked:', ctaPosition);
  };

  const handlePrimaryClick = (ctaPosition = 'final') => {
    trackCTAClick(ctaPosition);
    // CHANGEMENT CRITIQUE : Redirection directe vers l'offre Genesis Revival
  ...
  // Données des témoignages AVEC SCORES AVANT/APRÈS
  const testimonials = [
    {
      id: 1,
      name: "Sarah M.",
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className="text-xs font-semibold tracking-wide uppercase bg-gradient-to-r from-rose-100 to-pink-50 text-rose-800 px-3 py-1.5 rounded-full border border-rose-300">
              ⏰ Your Personalized Analysis Reserved for {formatTime(timeLeft)}
            </span>
          </motion.div>
          ...
          >
            ...
            {/* ═══════════ NOUVEAU CTA #1 - APRÈS RÉVÉLATION ═══════════ */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
            ...
            >
              ...
              <p className="text-xs text-center text-gray-600 mt-2">
                Discover the 9-compound formula • Takes 30 seconds
              </p>
            </motion.div>
            ...
            {/* ═══════════ NOUVEAU CTA #2 - APRÈS 9 COMPOUNDS ═══════════ */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* WHAT THIS MEANS FOR YOU (INSIGHTS PERSONNALISÉS) */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            {/* ═══════════ NOUVEAU CTA #3 - APRÈS TÉMOIGNAGES ═══════════ */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="bg-gradient-to-br from-amber-50 to-orange-50/50 rounded-xl p-6 border border-amber-300 max-w-2xl mx-auto">
                ...
                <p className="text-xs text-gray-600 mt-3">
                  Same {scoreCategory?.pattern} pattern as you • 94% success rate
                </p>
              </div>
            </motion.div>
          </motion.div>
